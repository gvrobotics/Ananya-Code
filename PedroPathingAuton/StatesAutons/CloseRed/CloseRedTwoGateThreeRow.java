package org.firstinspires.ftc.teamcode.PedroPathingAuton.StatesAutons.CloseRed;
...
@Autonomous(group = "Close Red", name = "1. Close Red 2 Gate 3 Row", preselectTeleOp = "Tele 5440")
public class CloseRedTwoGateThreeRow extends OpMode {
    ...
    private final Pose firstRowStart = new Pose(rowStart, firstRowPos, Math.toRadians(0));
    private final Pose firstRowEnd = new Pose(128, firstRowPos, Math.toRadians(0));
    private final Pose secondRowStart = new Pose(rowStart, secondRowPos, Math.toRadians(0));
    private final Pose move = new Pose(95.7, 73.1, Math.toRadians(0));
    private final Pose gate = new Pose(129.4, 67.4, Math.toRadians(0));
    private final Pose gateOut = new Pose(114.6, 67.4, Math.toRadians(0));
    private final Pose secondRowEnd = new Pose(rowEnd, secondRowPos, Math.toRadians(0));
    ...
    private PathChain driveStartPosShootPos, driveShootPosFirstRowStartPose,
            ...
            driveShootPosThirdRowStartPose, driveThirdRowStartToEndPose, driveThirdRowToGatePose;
    ...
    public void buildPaths() {
        ...
        driveSecondRowToShootPose = follower.pathBuilder()
                .addPath(new BezierLine(secondRowEnd, gateOut))
                .setLinearHeadingInterpolation(secondRowEnd.getHeading(), gateOut.getHeading())
                .addPath(new BezierLine(gateOut, gate))
                .setLinearHeadingInterpolation(gateOut.getHeading(), gate.getHeading())
                .setTimeoutConstraint(300)
                .addPath(new BezierLine(gate, move))
                .setLinearHeadingInterpolation(gate.getHeading(), move.getHeading())
        ...
        driveFirstRowtoShootPose = follower.pathBuilder()
                .addPath(new BezierLine(firstRowEnd, gateOut))
                .setLinearHeadingInterpolation(firstRowEnd.getHeading(), gateOut.getHeading())
                .addPath(new BezierLine(gateOut, gate))
                .setLinearHeadingInterpolation(gateOut.getHeading(), gate.getHeading())
                .setTimeoutConstraint(300)
                .addPath(new BezierLine(gate, move))
        ...
        driveThirdRowToGatePose = follower.pathBuilder()
                .addPath(new BezierLine(thirdRowEnd, gateOut))
                .setLinearHeadingInterpolation(thirdRowEnd.getHeading(), gateOut.getHeading())
                .build();
    }
    ...
    public void statePathUpdate() {
        switch (pathState) {
            ...
            case DRIVE_SECOND_ENDPOS_SHOOTPOS:
                follower.followPath(driveSecondRowToShootPose, true); // opens gate during this
            ...
            case DRIVE_FIRST_ENDPOS_SHOOTPOS:
                follower.followPath(driveFirstRowtoShootPose, true); // opens gate during this
            ...
            case DRIVE_THIRDROW_START_TO_END:
                handleRowCollection(driveThirdRowStartToEndPose, PathState.DRIVE_THIRD_ENDPOS_GATEPOS);
                break;

            case DRIVE_THIRD_ENDPOS_GATEPOS:
                follower.followPath(driveThirdRowToGatePose, true);
