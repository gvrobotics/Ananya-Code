...
public class Tele_3 extends OpMode {
    ...
    private Boolean flywheelDirection = false, prevUp = false, currUp = false;
    ...
    public void loop() {
        ...
        if (currentGamepad && !previousGamepad) {
            intakeOn = !intakeOn;
            double vel = intakeOn ? 1500 : 0;
            intake1.setVelocity(vel);
            intake2.setVelocity(vel);
        }
        ...
        if (launchState == LaunchState.IDLE && currentA && prevA) {
            pushOn = !pushOn;
            pushOn = !pushOn;
            push1.setPosition(pushOn ? 0.5 : 0.1); // up - down
        }
        prevA = currentA;
        ...
        currentGamepad3 = gamepad2.dpad_down;
        ...
        prevUp = currUp;
        currUp = gamepad2.dpad_up;

        if (currUp && !prevUp) {
            flywheelDirection = !flywheelDirection;
            if (flywheelDirection) {
        ...
        telemetry.addData("Flywheel Direction (Dpad Up)", intakeDirection ? "REVERSE" : "FORWARD");
        telemetry.addData("Intake Direction (Dpad Down)", flywheelDirection ? "FORWARD" : "REVERSE");
        telemetry.addLine();
        ...
        telemetry.addData("Angle (x)", launch.getPosition());
